...
export default async function MentorResourcesPage() {
  ...
  const { data: assets } = await supabase
    ...
    .contains("visibility_scope", ["mentor"])
    .order("lesson_number", { ascending: true, nullsFirst: false })
    .order("sort_order", { ascending: true });

  return (
    <Shell title="Resources" introKey="mentor.resources">
            ...
            <div key={asset.id} className="flex items-center justify-between rounded-xl border border-white/8 bg-white/3 px-5 py-4">
              ...
              {asset.url && (
                <a
                  ...
                  className="flex items-center gap-1 text-xs text-vivid-teal hover:underline"
                >

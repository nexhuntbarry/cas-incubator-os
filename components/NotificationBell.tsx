      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 bg-deep-navy border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden">
          <div className="px-4 py-3 border-b border-white/8 flex items-center justify-between">
            <p className="text-sm font-semibold text-soft-gray">{t("bell")}</p>
            {unreadCount > 0 && (
              <span className="text-xs text-electric-blue">{t("unreadCount", { count: unreadCount })}</span>
            )}
          </div>

          <div className="max-h-80 overflow-y-auto">
            {loading ? (
              <div className="px-4 py-6 text-center text-sm text-soft-gray/40">{t("loading")}</div>
            ) : notifications.length === 0 ? (
              <div className="px-4 py-6 text-center text-sm text-soft-gray/40">{t("noNotifications")}</div>
            ) : (
              notifications.map((n) => (
                <button
                  key={n.id}
